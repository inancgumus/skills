#!/usr/bin/env python3
"""Deterministic slop pre-flagger for the write skill.

Adapted from an MIT-licensed detector, Copyright (c) 2026 Timur Isachenko.
...
rubric; this only surfaces the obvious offenders fast.

Two surfaces:
  * flag(text)  -> list[{line, type, pattern, span}]   (line-oriented candidates)
  * score(text) -> {v, paragraphs[...], overall{...}}   (per-paragraph slop_band)

`flag()` and the default CLI keep a stable hit shape. Severity scoring is
...
Usage:
    python3 flag_patterns.py FILE
    cat text | python3 flag_patterns.py
    python3 flag_patterns.py --score FILE      # per-paragraph slop_band JSON
    python3 flag_patterns.py --profile strict FILE   # aggressive opt-in rules
    python3 flag_patterns.py --selftest
...
# Adding a rule? Give it a weight in _WEIGHTS.
_RULES = [
    ...
    ("intensifier_degree", "degree intensifier (often legit)",
     ...
     re.compile(r"\b(?i:very|really|extremely|incredibly|quite|rather)\s+"
                ...
                r"few\b|little\b|more\b|than\b))(?![A-Z])[a-z]\w+")),
    ...
    # Patterns below are kept narrow + idiom-anchored so honest technical prose
    # stays silent (the low-false-positive rule), unlike a blanket banned-word
    # match. See references/patterns.md for the taxonomy.
    ...
    # --- assistant-voice and structural-cadence tells ---
    ...
    ("leadinlabel", "lead-in label",
     re.compile(r"(?mi)^\s*\*{0,2}(bottom line|key (insight|takeaway)|"
                r"the (result|takeaway)|net[- ]net|tl;?dr)\*{0,2}\s*:")),
    ("selfref", "text referring to itself",
     re.compile(r"\bthis (pr|pull request|doc|document|post|article|change|"
                r"commit|patch|section) (does|adds|introduces|fixes|explains|"
                r"covers|implements|provides)\b", re.I)),
    ("mdseparator", "markdown separator rule",
     re.compile(r"^\s*(-{3,}|\*{3,}|_{3,})\s*$")),
    # record-keeping phrasing; "on record as ..." is the human journalistic
    # idiom ("she is on record as opposing"), so it is excluded, and the bare
    # weather idiom ("hottest summer on record") stays clean because the
    # no-form requires a leading "no"
    ("reportspeak", "record-keeping phrasing (report register)",
     re.compile(r"\b(recorded occurrences?|"
                r"(is|are|remains?) on record(?!\s+as\b)|"
                r"no \w+ (?:is )?on record|"
                r"\d+-count)\b", re.I)),
]
...
def _load_blocklist():
    """Compile scripts/blocklist.txt into one regex; None if absent/empty.

    Line format: term per line, '#' comments, trailing '*' = any word ending,
    spaces in multiword terms also match hyphens. Kept as a plain text file so
    the list is editable without touching this script.
    """
    ...
    for raw in lines:
        ...
        for tok in term.split():
            ...
            if not tok:
                continue
            toks.append(re.escape(tok).replace("'", "['’]")
                        + (r"\w*" if star else ""))
        if not toks:
            continue
        pat = r"\b" + r"[\s-]+".join(toks)
        if not toks[-1].endswith(r"\w*"):
            pat += r"\b"
        parts.append(pat)
    if not parts:
        return None
...
_BLOCKLIST_RE = _load_blocklist()
if _BLOCKLIST_RE is not None:
    _RULES.append(("blocklist", "blocklist term (scripts/blocklist.txt)",
                   _BLOCKLIST_RE))

# Aggressive, opt-in rules for `--profile strict`: bans every -ly adverb, any
# em-dash, and Wh- question openers outright. Kept OFF by default because a
# lone adverb or em-dash is normal in honest prose; switching them on still
# keeps the flag-only report + over-correction guardrails on top.
_PROFILE_EXTRA_RULES = {
    "strict": [
        ("adverb_ly", "-ly adverb (strict profile bans all)",
         re.compile(r"\b\w{3,}ly\b", re.I)),
        ("wh_opener", "Wh- question opener (strict profile bans)",
         re.compile(r"(?mi)^\s*(what|when|where|which|who|why|how)\b")),
        ("emdash_any", "any em-dash (strict profile bans)", re.compile(r"—")),
    ],
}
_PROFILE_EXTRA_WEIGHTS = {
    "strict": {"adverb_ly": 4, "wh_opener": 8, "emdash_any": 8},
}
...
_WEIGHTS = {
    ...
    "leadinlabel": 12, "selfref": 10, "mdseparator": 8, "reportspeak": 8,
    "blocklist": 6,
...
def _rules_for(profile):
    """Active rule list for a profile. Default = the low-false-positive set."""
...
def flag(text, profile="default"):
    """Line-oriented slop candidates. Output shape is stable across versions.

    `profile` selects the rule set: "default" keeps the low-false-positive
    contract; "strict" adds the aggressive opt-in rules (all adverbs, Wh-
    openers, any em-dash).
    """
    hits = []
    for i, line in enumerate(text.splitlines(), 1):
    ...
    # Frontmatter delimiters and setext-heading underlines are syntax, not
    # separator rules. Frontmatter cannot contain blank lines, so the scan
    # for its closer stops at the first blank; a hyphen line directly under
    # a non-blank line is a setext heading, not a separator.
    fm_lines = text.splitlines()
    fm_end = 0
    if fm_lines and fm_lines[0].strip() == "---":
        for j in range(1, len(fm_lines)):
            if not fm_lines[j].strip():
                break
            if fm_lines[j].strip() in ("---", "..."):
                fm_end = j + 1
                break

    def _is_setext_underline(line_no):
        return (line_no >= 2 and fm_lines[line_no - 2].strip()
                and set(fm_lines[line_no - 1].strip()) == {"-"})

    hits = [h for h in hits
            if not (h["type"] == "mdseparator"
                    and (h["line"] <= fm_end or _is_setext_underline(h["line"])))]
...
def _paragraphs(text):
    ...
    first line opens a heading, blockquote, or list: the same scope rules
    SKILL.md declares (frontmatter and link targets stay with the model).
    """
...
_SELFTEST = (
    ...
    "Moreover, the system is robust.\n"                # 3  transition + blocklist(robust); empower SILENT
    ...
    "The pipeline is robust and we leverage caching for speed.\n"  # 13 blocklist(robust, leverage); empower SILENT (riders, no marketing)
    ...
    "Bottom line: the cache saves money.\n"  # 26 leadinlabel
    "This PR adds retry logic to the uploader.\n"  # 27 selfref
    "The log shows 7 recorded occurrences of the crash.\n"  # 28 reportspeak
    "A battle-tested, production ready fix with impeccable hygiene.\n"  # 29 blocklist
    "\n"  # 30 blank, so the next line is a separator, not a setext underline
    "---\n"  # 31 mdseparator (not a frontmatter delimiter)
)
...
def _selftest():
    ...
    expected = {"hedge", "listicle", "transition", "stakes", "candor",
                ...
                "copula", "hedgestack", "leadinlabel", "selfref",
                "mdseparator", "reportspeak", "blocklist"}
